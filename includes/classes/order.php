...
// TVA_INTRACOM BEGIN
  ...
  function zen_tva_geo_tax($tax_country_id, $billing_company, $billing_tva_intracom){
    ...
    if($tax_country_id == STORE_COUNTRY ) {
      return true; // store
    }
    if($tax_geo_zone_id == 0) {
      return true; // no geo-zone
    }
    if($tax_geo_zone_id != 0 && !$billing_tva_intracom) {
      return true; // geo-zone & no vat
    } elseif(!$billing_company && !$billing_tva_intracom) {
      return false; // no vat
    } else {
      return false;
    }
